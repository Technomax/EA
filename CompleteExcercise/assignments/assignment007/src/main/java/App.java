...
public class App {
    ...
    static {
        sessionFactory = DBConnectionFactory.getSessionFactory(Arrays.asList(Airline.class, Airplane.class, Airport.class, Flight.class));
    }
    ...
    public static void printTask01() {
        DBConnectionFactory.apply(session -> {
            System.out.println("a) Flights leaving USA capacity > 500");
            System.out.println("=======================================");
            List<Flight> flights = session.createQuery("SELECT a FROM Flight a join a.origin b join a.airplane c " +
                    "where b.country='usa' and c.capacity>500").list();
    ...
    public static void printTask02() {
        DBConnectionFactory.apply(session -> {
            System.out.println("b) All airlines that use A380 airplanes");
            System.out.println("=======================================");
            List<Airline> airlines = session.createQuery("select distinct a from Airline a join a.flights b join b.airplane c " +
                    "where c.model='A380'").list();
    ...
    public static void printTask03() {
        DBConnectionFactory.apply(session -> {
            System.out.println("c) Flights using 747 planes that don't belong to Star Alliance");
            System.out.println("===============================================================");
            List<Flight> flights = session.createQuery("select a from Flight a join a.airplane b join a.airline c " +
                    "where b.model='747' and c.name!='Star Alliance'").list();
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:");
            for (Flight flight : flights) {
                System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s\n",
    ...
    public static void printTask04() {
        DBConnectionFactory.apply(session -> {
            System.out.println("d) All flights leaving before 12pm on 08/07/2009");
            System.out.println("==================================================");
            List<Flight> flights = session.createQuery("select a from Flight a where a.departureDate='2009-08-07' and a.departureTime<'12:00:00'").list();
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:");
            for (Flight flight : flights) {
                System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s\n",
